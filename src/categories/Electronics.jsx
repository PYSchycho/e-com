    ...
    useEffect(() => {
        const fetchElectronics = async () => {
            try {
                ...
                if (!response.ok) {
                    throw Error('Failed to fetch Electronics');
                }
        ...
        <div>
            ...
            <div>
                {loading ? (
                    ...
                    <div className="py-3 grid lg:grid-cols-6 gap-4 sm:grid-cols-4 md:grid-cols-3 grid-cols-2 xl-grid-cols-6 2xl:grid-cols-3 shadow-xl ">
                        {filterProducts.map(product => (
                            <div key={product.id} className="border p-2 rounded-lg shadow-lg">
                                <img
                                    src={product.image}
                                    alt={product.title}
                                    className="w-48  h-48 border-2 mb-2 rounded-md " />
